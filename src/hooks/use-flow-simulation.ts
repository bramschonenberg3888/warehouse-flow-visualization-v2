...
export interface FlowPalletState {
  flowId: string
  x: number
  y: number
  progress: number
...
export interface UseFlowSimulationResult {
...
export function useFlowSimulation(
  ...
  // Animation refs to avoid re-renders during animation
  ...
  // Initialize flow animations from current data (called on demand, not in effect)
  ...
  // Animation effect - runs when isRunning changes
  useEffect(() => {
  ...
  const start = useCallback(() => {
    ...
    // Initialize flows when starting
    const pallets = initializeFlows()
    if (flowAnimationsRef.current.size === 0) return

    isRunningRef.current = true

    // Reset start times for all flows
    flowAnimationsRef.current.forEach((anim) => {
      anim.startTime = 0
    })
  ...
  const reset = useCallback(() => {
    ...
    // Re-initialize and reset pallets to starting positions
    const pallets = initializeFlows()
    setState({ isRunning: false, pallets })
  }, [stop, initializeFlows])
